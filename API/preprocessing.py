...
def for_evaluate(file, scale=0, create_file=False):
    ...
    if scale == "EURUSD" or scale == "GBPUSD":
        label = label*10000
    elif scale == "USDJPY":
        label = label*100
...
def for_predict(file):
    ...
    df = data.iloc[:-24,:].copy(deep=False)
